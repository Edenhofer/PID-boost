...
Hence by plotting the purity over the likelihood ratio one should expect to observe a monotonically increasing function. An idealized version of such a graph is depicted in \autoref{fig:neyman_pearson_visualization}. Since the underlying data may not be assumed to be a continues stream the likelihood ratio has been binned as it better represents the actual shape to be expected.
